...
def image_upload():
    if request.method == "POST":
        ...
        blur_g = np.array(
            [[min(max(np.sum(filter_matrix * grn_arr[3 * i:3 * i + 3, 3 * j:3 * j + 3]), 0), 255) for j in range(400)]
             for i in range(400)], dtype=np.int8)
        out_green = Image.fromarray(blur_g, 'L')

        blur_r = np.array(
            [[min(max(np.sum(filter_matrix * red_arr[3 * i:3 * i + 3, 3 * j:3 * j + 3]), 0), 255) for j in range(400)]
             for i in range(400)], dtype=np.int8)
        out_red = Image.fromarray(blur_r, 'L')

        blur_b = np.array(
            [[min(max(np.sum(filter_matrix * blue_arr[3 * i:3 * i + 3, 3 * j:3 * j + 3]), 0), 255) for j in range(400)]
             for i in range(400)], dtype=np.int8)
